...
SVC_Handler     PROC
				...
				; add three lines of code here to call _systemcall_table_jump
				IMPORT _systemcall_table_jump 	; import _systemcall_table_jump
				PUSH	{LR}				  	; push our current link register before we branch into
												; _systemcall_table_jump to return back later
				BL 	   _systemcall_table_jump	; call _systemcall_table_jump
				; do not update the following two lines
                MRS	R1, PSP						; added for CSS422 project
				STR	R0, [R1]					; added for CSS422 project
				; add one line of code here to return from _systemcall_table_jump
				POP		{PC}					; return to our previous operation
                ENDP
DebugMon_Handler\
                PROC
