...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		// instantiate a file system (must do before synchronized queues)
		...
		while ( disk.testAndResetReady( ) == false )
		    ioQueue.enqueueAndSleep( COND_DISK_FIN );
		return OK;
	    case READ: // Kernel.READ can be called from either SysLib.cin or SysLib.read
		switch ( param ) {
		case STDIN:
		    try {
		...
		if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
			return fs.read( myTcb.getFtEnt( param ), ( byte[] )args ); // once realize that fd is not 0, 1, or 2,
																	   // know that it is either invalid (less
																	   // than 0) or valid with args type of byte[]
		}
		return ERROR;
	    case WRITE:
		switch ( param ) {
		...
		case STDOUT:
		    ...
		    return OK; // break; // should be a return because fd has been found
		case STDERR:
		...
		if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
			...
			return ftEnt != null ? fs.write( myTcb.getFtEnt( param ), ( byte[] )args )
								 : ERROR; // once realize that fd is not 0, 1, or 2,
								 		  // know that it is either invalid (less than
										  // 0) or valid with args type of byte[]
		}
		return ERROR;
		...
		if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
			...
			// create new FileTableEntry for given filename and mode
			FileTableEntry ent = fs.open( s[0], s[1] );
			// allocate and return a new fd for FileTableEntry that was created
		...
		if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
			// get FileTableEntry corresponding to given fd
			FileTableEntry ent = myTcb.getFtEnt( param );
			// close this FileTableEntry
			if (fs.close( ent )) {
				// make the fd available for use by other files
				myTcb.returnFd( param );
				return OK;
			} else
				// return -1 if failed to close FileTableEntry
				return ERROR;
		} else
			return ERROR;
		...
		if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
			...
			// get FileTableEntry corresponding to given fd
			FileTableEntry ent = myTcb.getFtEnt( param );
			// update seek pointer of FileTableEntry and
			// return new seek pointer or -1 on failure
			return fs.seek( ent, i[0], i[1] );
		} else
			return ERROR;
	    case FORMAT:  // to be implemented in project
		// format file system with given number of files;
		// assume that directory is always included and thus
		// do not allow nonpositive numbers of files to be used
		return param > 0 && fs.format( param ) ? OK : ERROR;
	    case DELETE:  // to be implemented in project
		// get String from args; delete file with given filename
